...
public class Complex{
	...
	private double x;
	private double y;
	private double setReal;
	private double setImg;
	...
	//polar form
	public ArrayList<Double> polarComplex(Complex V){
		double magn= Math.sqrt(Math.pow(V.getReal(),2)+ Math.pow(V.getImg(),2));
		double thet= Math.atan((V.getImg()/V.getReal()));
		ArrayList polar= new ArrayList<Double>();
		polar.add(magn);
		polar.add(thet);
		return(polar);
	}
	...
	public double[] root(){
        ...
        double t= Math.cos((((Math.atan(y/x))/2)+(2*k*Math.PI))/n);
		double s= Math.sin((((Math.atan(y/x))/2)+(2*k*Math.PI))/n);
		double root= (t+s)*r;
		array[i]= root;
		n++;
		}
